...
func TestGet(t *testing.T) {
	...
	simp := NewGroup("scores", 2<<10, GetterFunc(
	...
	for k, v := range db {
		if view, err := simp.Get(k); err != nil || view.String() != v {
			t.Fatalf("failed to get value of Tom")
		}
		if _, err := simp.Get(k); err != nil || loadCounts[k] > 1 {
			t.Fatalf("cache %s miss", k)
		}
	}
	if view, err := simp.Get("unknown"); err == nil {
...
func TestGetGroup(t *testing.T) {
	...
	NewGroup(groupName, 2<<10, GetterFunc(
		func(key string) (b []byte, err error) { return }))
